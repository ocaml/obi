...
let bulk ({staging_hub_id; results_dir; _}) arch {ov; distro} opam_repo_rev () =
  ...
  let cmds =
    `A [
      ...
      `String (Fmt.strf "docker run %s:%s opam list --all-versions --installable -s | tail -2 > %s/pkgs.txt" staging_hub_id tag tag);
  ...
  let gather_cmds = `A [
    ...
    `String (Fmt.strf "git clone git@github.com:avsm/obi-logs out");
    `String (Fmt.strf "./obi-buildkite process -vv -i %s -o out" tag);
    `String (Fmt.strf "git -C out add . && git commit -m 'update %s' && git push" tag);
  ] in
