...
module Rev = struct
  ...
  type t = {rev: string; date: float; subject: string} [@@deriving sexp]

  let print_date t =
    Ptime.of_float_s t.date
    |> (function None -> failwith "bad time" | Some d -> d)
    |> Ptime.to_rfc3339

  let print_rev t = String.with_range ~len:8 t.rev
...
let load_maintainers dir =
  let m =
    try
      Sexplib.Sexp.load_sexp_conv_exn
        Fpath.(dir / "maintainers.sxp" |> to_string)
        Obi.Index.maintainers_of_sexp
    with _ -> []
  in
...
let load_tags dir =
  let m =
    try
      Sexplib.Sexp.load_sexp_conv_exn
        Fpath.(dir / "tags.sxp" |> to_string)
        Obi.Index.tags_of_sexp
    with _ ->
      Logs.info (fun l -> l "Unable to load tags cache") ;
      []
  in
...
let info_of_rev tdir rev =
  let run_git args =
    OS.Cmd.(run_out Cmd.(v "git" % "-C" % p tdir %% args) |> to_string)
  in
  run_git Cmd.(v "show" % "-s" % "--pretty=format:%ct" % rev)
  >>= fun date ->
  String.trim date |> float_of_string
  |> fun date ->
  run_git Cmd.(v "show" % "-s" % "--pretty=format:%s" % rev)
  >>= fun subj -> String.trim subj |> fun subject -> Ok {Rev.rev; date; subject}
...
let parse_string_list buf =
  ...
  | Some _ ->
      let rec fn ic =
        kscanf ic (fun _ _ -> []) "%S %r" fn (fun a b -> a :: b)
      in
...
let find_maintainers pkg =
  ...
  | None ->
      OS.Cmd.(
        run_out Cmd.(v "opam" % "info" % "-f" % "maintainer:" % pkg)
        |> to_string)
      >>= fun maintainer ->
...
let find_tags pkg =
  ...
  | None ->
      OS.Cmd.(
        run_out Cmd.(v "opam" % "info" % "-f" % "tags:" % pkg) |> to_string)
      >>= fun t ->
      let t' = String.trim t |> parse_string_list in
      Hashtbl.add tags pkg t' ; Ok t'

let find_log logs_dir params rev pkg version =
  let open Obi.Builds in
  let log =
    Printf.sprintf "%s/linux/%s/%s/%s/%s/%s.%s.txt" (Fpath.to_string logs_dir)
      (OV.string_of_arch params.arch)
      (D.tag_of_distro params.distro)
      (OV.to_string params.ov) rev pkg version
  in
  Bos.OS.File.read_lines (Fpath.v log)
  >>= fun lines ->
  ...
  let rev =
    List.find
      (fun rev ->
        ...
        |> List.find_opt (fun (p, _) -> p = params)
        |> function
        | None -> false
        | Some (_, b) ->
            r := Some b ;
      ...
      srevs
  in
  match !r with None -> failwith "distro not found" | Some v -> (rev, v)

let live_revs = Hashtbl.create 7

let get_opam_actions j =
  try Ezjsonm.from_string j |> Obi.OpamJsonActions.installs with _ -> []

let is_depfail name version actions =
  List.exists
    (fun (n, v, r) -> name = n && version = v && r = `Skipped)
    actions
      ...
      C.map
        (fun (version, res) ->
          ...
          | _ ->
            ...
            | Error (`Msg e) ->
                Logs.warn (fun l ->
                    l "Error log not found for %s.%s (%a): %s" name version
                      Bos.OS.Cmd.pp_status res.code e ) ;
                Ok [] )
          >>= fun log ->
          let build_result =
            match res.code with
            ...
            | `Exited n when not (is_depfail pkg.name version deps) ->
                `Fail (n, deps)
            | `Exited n -> `Depfail deps
          in
          let params =
            {Obi.Index.arch= params.arch; ov= params.ov; distro= params.distro}
          in
          let metadata =
            [ { Obi.Index.params
              ; rev
    ...
    (fun pkgs ->
      List.iter
        (fun pkg ->
          ...
          let maintainers =
            List.fold_left
              (fun a m -> if List.mem m a then a else m :: a)
              p.maintainers pkg.maintainers
          in
                  ...
                  >>= C.iter (fun distro ->
                          ...
                          >>= C.iter (fun ov ->
                                  ...
                                  >>= C.iter (fun rev ->
                                          ...
                                          if
                                            Hashtbl.mem live_revs
                                              (Fpath.to_string rev)
                                          then (
                                            Logs.debug (fun l ->
                                                l
                                                  "Logs repo %a is live, not \
                                                   deleting"
                                                  Fpath.pp rev ) ;
                                            Ok () )
                                          else (
                                            Logs.debug (fun l ->
                                                l
                                                  "Logs repo %a is not live, \
                                                   so deleting"
                                                  Fpath.pp dir ) ;
                                            Bos.OS.Dir.delete ~must_exist:true
                                              ~recurse:true dir ) ) ) ) ) )
  >>= fun () ->
  ...
  >>= C.iter (fun os ->
          ...
          >>= C.iter (fun arch ->
                  ...
                  >>= C.iter (fun distro ->
                          ...
                          >>= C.iter (fun ov ->
                                  let dir = Fpath.(dir // ov) in
                                  OV.of_string (Fpath.to_string ov)
                                  >>= fun ov ->
                                  OS.Dir.contents ~rel:true dir
                                  >>= C.iter (fun rev ->
                                          ...
                                          then (
                                            ...
                                            Logs.debug (fun l ->
                                                l
                                                  "Metadata file %a is not \
                                                   live, so deleting"
                                                  Fpath.pp file ) ;
                                            Bos.OS.File.delete ~must_exist:true
                                              file ) ) ) ) ) )
...
let summarise input_dir (opam_dir: Fpath.t) =
  ...
  >>= C.iter (fun os ->
          ...
          >>= C.iter (fun arch ->
                  let dir = Fpath.(dir // arch) in
                  Logs.info (fun l ->
                      l "Found CPU architecture %a" Fpath.pp arch ) ;
                  let arch = OV.arch_of_string_exn (Fpath.to_string arch) in
                  OS.Dir.contents ~rel:true dir
                  >>= C.iter (fun distro ->
                          Logs.info (fun l ->
                              l "Found distribution %a" Fpath.pp distro ) ;
                          let dir = Fpath.(dir // distro) in
                          let distro =
                            D.distro_of_tag (Fpath.to_string distro)
                            |> function
                            | None -> failwith "unknown distro" | Some d -> d
                          in
                          OS.Dir.contents ~rel:true dir
                          >>= C.iter (fun ov ->
                                  Logs.info (fun l ->
                                      l "Found OCaml version %a" Fpath.pp ov ) ;
                                  let dir = Fpath.(dir // ov) in
                                  OV.of_string (Fpath.to_string ov)
                                  >>= fun ov ->
                                  OS.Dir.contents ~rel:true dir
                                  >>= C.iter (fun rev ->
                                          Logs.info (fun l ->
                                              l "Found git rev %a" Fpath.pp rev
                                          ) ;
                                          let file =
                                            Fpath.(dir // rev |> to_string)
                                          in
                                          let params =
                                            {Obi.Builds.arch; distro; ov}
                                          in
                                          let batch =
                                            Sexplib.Sexp.load_sexp_conv_exn
                                              file Obi.Builds.batch_of_sexp
                                          in
                                          ( if Hashtbl.mem revs batch.rev then
                                            Ok ()
                                          else
                                            info_of_rev opam_dir batch.rev
                                            >>= fun info ->
                                            Hashtbl.replace revs batch.rev info ;
                                            Ok () )
                                          >>= fun () ->
                                          Hashtbl.add h batch.rev
                                            (params, batch) ;
  ...
  let srevs =
    ...
    |> List.sort (fun a b -> compare b.Rev.date a.Rev.date)
  in
  List.iter
    (fun r ->
      let open Rev in
      Printf.eprintf "%s %s %s\n%!"
        (String.with_range ~len:8 r.rev)
        (print_date r) r.subject )
    srevs ;
  Printf.eprintf "\n%!" ;
  ...
  let all_params =
    Hashtbl.fold
      (fun k (_, v) a -> if List.mem v.params a then a else v.params :: a)
      h []
  in
  ...
  let latest =
    List.map
      (fun params ->
        ...
        |> List.sort (fun a b ->
               Rev.compare (Hashtbl.find revs a.rev) (Hashtbl.find revs b.rev)
           )
        |> List.find (fun b -> b.params = params) )
      all_params
  in
  C.map (pkg_metadata_of_batch logs_dir) latest
  >>= fun latest ->
