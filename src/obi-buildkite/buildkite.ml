...
let retry () = ("retry", `O [("automatic", `Bool true)])

let bulk {staging_hub_id; results_dir; prod_hub_id} arch {ov; distro}
    opam_repo_rev () =
  ...
  let dfiles =
    ...
    @@ copy ~src:[".buildkite/obi-ci-install.sh"]
         ~dst:"/usr/bin/obi-ci-install" ()
    @@ run "sudo chmod a+x /usr/bin/obi-ci-install"
    @@ run "opam-sandbox-enable"
  in
  let tag =
    Fmt.strf "bulk-%s-%s-linux-%s-%s" (D.tag_of_distro distro)
      (OV.to_string ov |> String.map (function '+' -> '-' | x -> x))
      (OV.string_of_arch arch) opam_repo_rev
  in
  let label =
    Fmt.strf "Bulk %s %s %s: %s" (D.tag_of_distro distro) (OV.to_string ov)
      (OV.string_of_arch arch) opam_repo_rev
  in
  ...
  let bulk_tmpl =
    let cmds =
      `A
        [ `String
            (Fmt.strf "./.buildkite/obi-batch-install.sh %s %s %s __PKG__"
               staging_hub_id tag (OV.string_of_arch arch)) ]
    in
      ...
      [ ( "steps"
        , `A
            [ `O
                ...
                ; ("label", label)
                ; retry ()
                ; docker_agents (OV.string_of_arch arch) ] ] ) ]
  in
  ignore
    (Bos.OS.File.write
       Fpath.(dir / "template.yml")
       (Yaml.to_string_exn bulk_tmpl)) ;
  let cmds =
    `A
      [ `String
          (Fmt.strf "./.buildkite/obi-list-all-pkgs.sh %s %s %s" staging_hub_id
             tag opam_repo_rev) ]
  in
  let p1_builds =
    `O
  ...
  let gather_cmds =
    `A
      [ `String
          (Fmt.strf "./.buildkite/obi-gather-results.sh %s %s %s %s %s" tag
             (OV.string_of_arch arch) (OV.to_string ov)
             (D.tag_of_distro distro) opam_repo_rev) ]
  in
  let gather =
    [ `O
        [ ("command", gather_cmds)
        ; retry ()
        ; ("agents", `O [("githubpusher", `Bool true)])
        ; ("label", `String "Gather Results") ] ]
  in
  let yml = `O [("steps", `A (p1_builds :: `String "wait" :: gather))] in
  Bos.OS.File.write
    Fpath.(results_dir / "bulk.yml")
    (Yaml.to_string_exn ~len:256000 yml)
...
let gen ({staging_hub_id; prod_hub_id; results_dir; _} as opts) () =
  ...
  List.iter
    (fun (f, arch) ->
      ...
      | exception Not_found -> Hashtbl.add p2 f [arch] )
    p1 ;
  let p1_builds =
    List.map
      (fun (f, arch) ->
        ...
        let cmds =
          `A
            [ `String
                (Fmt.strf
                   "buildkite-agent artifact download phase1-%s/Dockerfile.%s ."
                   arch f)
            ; `String
                (Fmt.strf
                   "docker build --no-cache --rm --pull -t %s -f \
                    phase1-%s/Dockerfile.%s ."
                   tag arch f)
    ...
    Hashtbl.fold
      (fun f arches acc ->
        let tags =
          List.map
            (fun arch ->
              Fmt.strf "%s:%s-opam-linux-%s" staging_hub_id f
        ...
        `O
          ( [ ("command", cmds)
            ...
            ; docker_agents "amd64"
            ; docker_login ]
          @ concurrency 12 "containers/ocaml" )
        :: acc )
      p2 []
  in
  let p3 =
      ...
      (fun (f, arch) ->
        ...
        let cmds =
          `A
            [ `String
                (Fmt.strf
                   "buildkite-agent artifact download phase3-%s/Dockerfile.%s ."
                   arch f)
            ; `String
                (Fmt.strf
                   "docker build --no-cache --rm --pull -t %s -f \
                    phase3-%s/Dockerfile.%s ."
                   tag arch f)
            ; `String (Fmt.strf "docker push %s" tag) ]
        in
        `O
  ...
  List.iter
    (fun (f, arch) ->
      ...
      | exception Not_found -> Hashtbl.add p4 f [arch] )
    p3 ;
  let p3_march =
    Hashtbl.fold
      (fun f arches acc ->
        ...
        let tags =
          List.map
            (fun arch ->
              Fmt.strf "%s:%s-linux-%s" staging_hub_id f
                (OV.string_of_arch arch) )
            arches
        in
        let l = String.concat " " tags in
        ...
        let annotates =
          List.map2
            (fun tag arch ->
              `String
                (Fmt.strf "docker manifest annotate %s:%s %s --arch %s"
                   prod_hub_id f tag (OV.string_of_arch arch)) )
            tags arches
        in
        let cmds =
          `A
            ( pulls
            @ [ `String
                  (Fmt.strf "docker manifest push -p %s:%s || true" prod_hub_id
                     f)
              ; `String
                  (Fmt.strf "docker manifest create %s:%s %s" prod_hub_id f l)
              ]
            @ annotates
            @ [ `String (Fmt.strf "docker manifest inspect %s:%s" prod_hub_id f)
              ; `String
                  (Fmt.strf "docker manifest push -p %s:%s" prod_hub_id f) ] )
        in
        `O
          ( [ ("command", cmds)
            ; ("label", `String label)
        ...
        let annotates =
          List.map2
            (fun t arch ->
              `String
                (Fmt.strf "docker manifest annotate %s:%s %s --arch %s"
                   prod_hub_id tag t (OV.string_of_arch arch)) )
            tags arches
        in
        let cmds =
          `A
            ...
            @ annotates
            @ [ `String
                  (Fmt.strf "docker manifest inspect %s:%s" prod_hub_id tag)
              ; `String
  ...
  let p6_march =
    ...
    let tags =
      List.map
        (fun arch ->
          Fmt.strf "%s:%s-linux-%s" staging_hub_id f (OV.string_of_arch arch)
          )
        arches
    in
    let l = String.concat " " tags in
    ...
    let annotates =
      List.map2
        (fun t arch ->
          `String
            (Fmt.strf "docker manifest annotate %s:%s %s --arch %s" prod_hub_id
               tag t (OV.string_of_arch arch)) )
        tags arches
    in
    let cmds =
      `A
  ...
  let yml =
    `O
      [ ( "steps"
        , `A
            ( p1_builds @ wait @ p2_march @ wait @ p3_builds @ wait @ p3_march
            @ p4_march @ p5_march @ p6_march ) ) ]
  in
  Bos.OS.File.write
    Fpath.(results_dir / "phase1.yml")
    (Yaml.to_string_exn ~len:256000 yml)
...
let process input_dir output_dir () =
  let open Bos in
  OS.File.read Fpath.(input_dir / "rev")
  >>= fun rev ->
  OS.File.read Fpath.(input_dir / "arch")
  >>= fun arch ->
  OS.File.read Fpath.(input_dir / "distro")
  >>= fun distro ->
  OS.File.read Fpath.(input_dir / "ov")
  >>= fun ov ->
  let open Obi in
  let rev = String.trim rev in
  let arch = String.trim arch |> OV.arch_of_string_exn in
  let distro =
    String.trim distro |> D.distro_of_tag
    |> function Some x -> x | None -> failwith "invalid distro"
  in
  let ov = String.trim ov |> OV.of_string_exn in
  let logs = Fpath.(input_dir / "results") in
  OS.Dir.contents ~rel:true logs
  >>= fun pkgs ->
  let pkgs = List.filter (fun p -> Fpath.has_ext ".txt" p) pkgs in
  let odir =
    Fmt.strf "%a/metadata/linux/%s/%s/%s" Fpath.pp output_dir
      (OV.string_of_arch arch) (D.tag_of_distro distro) (OV.to_string ov)
    |> Fpath.v
  in
  OS.Dir.create ~path:true odir
  >>= fun _ ->
  let ldir =
    Fmt.strf "%a/logs/linux/%s/%s/%s/%s" Fpath.pp output_dir
      (OV.string_of_arch arch) (D.tag_of_distro distro) (OV.to_string ov) rev
    |> Fpath.v
  in
  OS.Dir.create ~path:true ldir
  >>= fun _ ->
  let h = Hashtbl.create 1000 in
  C.iter
    (fun pkg ->
      Logs.info (fun l -> l "Reading %a" Fpath.pp pkg) ;
      OS.File.read_lines Fpath.(logs // pkg)
      >>= fun lines ->
      let exit_code, start_time, end_time =
        ...
        with exn ->
          Logs.err (fun l ->
              l "ERROR parsing job output (%a %d %s)" Fpath.pp pkg
                (List.length lines) (Printexc.to_string exn) ) ;
          (130, 0., 0.)
      in
      let json_file = Fpath.(logs // pkg |> set_ext "json") in
      OS.File.read json_file
      >>= fun actions ->
      ( match exit_code with
      | 0 -> ()
      | n -> ignore (OS.File.write_lines Fpath.(ldir // pkg) lines) ) ;
      let res =
        {Builds.code= `Exited exit_code; start_time; end_time; actions}
      in
      pkg_version (Fpath.rem_ext pkg |> Fpath.to_string)
      >>= fun (name, version) ->
  ...
  let pkgs =
    Hashtbl.fold
      (fun name versions acc ->
        let versions =
          List.sort
            (fun a b -> Obi.VersionCompare.compare (fst a) (fst b))
            versions
        in
  ...
  Logs.info (fun l ->
      l "arch %s distro %s" (OV.string_of_arch arch) (D.tag_of_distro distro)
  ) ;
  let params = {Obi.Builds.arch; distro; ov} in
  let batch = {Obi.Builds.rev; params; pkgs} in
  Logs.info (fun l -> l "Writing %a" Fpath.pp ofile) ;
  OS.File.write ofile
    (Sexplib.Sexp.to_string_hum (Obi.Builds.sexp_of_batch batch))

let gen_summary input_dir opam_dir () = Import.summarise input_dir opam_dir
...
let bulk_build =
  ...
  let man =
    [`S Manpage.s_description; `P "Perform a bulk build of the opam packages"]
  in
  ( (let open Term in
    term_result
      (const bulk $ copts_t $ arch $ build_t $ opam_repo_rev $ setup_logs))
...
let summarise_cmd =
  ...
  let opam_dir =
    ...
    let open Arg in
    value
    & opt fpath (Fpath.v "opam-repository")
    & info ["r"; "opam-repo"] ~docv:"OPAM_REPO" ~doc
  in
  let man =
    [`S Manpage.s_description; `P "summarise stats from set of indexes."]
  in
